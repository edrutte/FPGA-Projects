...
void preProcess(char *Message, uint32_t Len, Chunk_t ca[16], ChunkNum_t ChunkNums) {
	for (uint32_t i = 0; i < ChunkNums.NumChunks; i++) {
        Chunk_t Chunk;
        uint8_t MSize;
        if (i != ChunkNums.OverflowChunk) {
            MSize = (uint8_t) (Len - (64 * i)) > 64 ? 64 : (Len - (64 * i));
        } else {
            MSize = 0;
        }
        if (MSize == 64) {
...
Schedule_t scheduleHash(const Chunk_t D, Schedule_t Schedule) {
    ...
    for (uint8_t i = 16; i < 64; i++) {
        ...
        uint32_t temp = (Schedule.Schedule[i - 16] + s0 + Schedule.Schedule[i - 7] + s1) & UINT32_MAX;
...
Hash_t hashCompress(Schedule_t Schedule, Hash_t LastHash) {
    ...
    for (uint8_t i = 0; i < 64; i++) {
        ...
        uint32_t ch = (e & f) ^ ((~e) & g);
        uint32_t temp1 = (h + S1 + ch + k[i] + Schedule.Schedule[i]) & UINT32_MAX;
        uint32_t S0 = rotr32(a, 2) ^ rotr32(a, 13) ^ rotr32(a, 22);
        uint32_t maj = (a & b) ^ (a & c) ^ (b & c);
        uint32_t temp2 = (S0 + maj) & UINT32_MAX;
        h = g;
        g = f;
        f = e;
...
Hash_t sha256chunk(const Chunk_t D, Hash_t LastHash) {
    ...
    LastHash.Hash[0] = (Compress.Hash[0] + LastHash.Hash[0]) & UINT32_MAX;
    LastHash.Hash[1] = (Compress.Hash[1] + LastHash.Hash[1]) & UINT32_MAX;
    LastHash.Hash[2] = (Compress.Hash[2] + LastHash.Hash[2]) & UINT32_MAX;
    LastHash.Hash[3] = (Compress.Hash[3] + LastHash.Hash[3]) & UINT32_MAX;
    LastHash.Hash[4] = (Compress.Hash[4] + LastHash.Hash[4]) & UINT32_MAX;
    LastHash.Hash[5] = (Compress.Hash[5] + LastHash.Hash[5]) & UINT32_MAX;
    LastHash.Hash[6] = (Compress.Hash[6] + LastHash.Hash[6]) & UINT32_MAX;
    LastHash.Hash[7] = (Compress.Hash[7] + LastHash.Hash[7]) & UINT32_MAX;
    return LastHash;
}
