...
entity sha256chunk_scheduleHash_Schedule_Schedule_2_buf_ram is 
    generic(
            DWIDTH     : integer := 32; 
            AWIDTH     : integer := 6; 
            MEM_SIZE    : integer := 64
...
architecture rtl of sha256chunk_scheduleHash_Schedule_Schedule_2_buf_ram is 
...
begin 
    if (clk'event and clk = '1') then
        if (ce16 = '1') then 
            q16 <= ram15(CONV_INTEGER(addr16_tmp));
        end if;
        if (ce0 = '1') then 
            if (we0 = '1') then 
                ram15(CONV_INTEGER(addr0)) := d0; 
            end if;
        end if;
    end if;
end process;
...
entity sha256chunk_scheduleHash_Schedule_Schedule_2_buf is
    generic (
...
architecture arch of sha256chunk_scheduleHash_Schedule_Schedule_2_buf is
    component sha256chunk_scheduleHash_Schedule_Schedule_2_buf_ram is
        port (
            clk : IN STD_LOGIC;
...
begin
    sha256chunk_scheduleHash_Schedule_Schedule_2_buf_ram_U :  component sha256chunk_scheduleHash_Schedule_Schedule_2_buf_ram
    port map (
        clk => clk,
        addr0 => address0,
        ce0 => ce0,
